...
function populateData(data){
  ...
  for(i = 0; i<reverse_data.length; i++){
    $('#foods-table').append(
      '<tr>'+
      '<td contenteditable="true" onblur = "rowEditVerification(this)" class="text-center name" onkeypress="return rowEditVerification(this, event)" id="name">' + data[i].name + '</td>' +
      '<td  contenteditable="true" onblur = "rowEditVerification(this)" class="text-center calories" onkeypress="return rowEditVerification(this, event)" id="calories">' + data[i].calories + '</td>' +
